(ns bacs-uk.main
  "Kotodama WASM entrypoint for the bacs_uk clean-room actor (L4) — Clojure port.
  ...
  facts (`bacs_uk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bacs_uk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bacs_uk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bacs_uk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bacs_uk-compat/ready)
